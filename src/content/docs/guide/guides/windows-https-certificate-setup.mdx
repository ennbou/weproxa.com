...
WePROXA shows the generated certificate path in **Settings** → **CA Certificate**. Use **Copy** to copy the path, then decide outside the app whether you want to import that certificate into Windows trust settings.

Newly generated certificates keep the **WePROXA Root CA** prefix and include their UTC generation date, computer hostname, and a short serial suffix. Use that full name to distinguish the current certificate from one copied from another computer or left behind after regeneration.
...
1. Open WePROXA and go to **Settings** → **CA Certificate**.
...
3. Use a browser or command-line client configured to `127.0.0.1:4545`.
